...
ggplot(split_exp, aes(x = Minus10, y=RNA_exp_average)) +
  ...
  labs(title = 'Distribution of Expression by -10', y = 'Expression', x = '-10 Variant') +
...
split_exp  %>% select(RNA_exp_average, Minus35, Minus10) %>% filter(RNA_exp_average > 0) %>%
  group_by_(.dots=c("Minus35", "Minus10")) %>% 
  summarize(RNA_exp_average = median(RNA_exp_average)) %>% ungroup() %>%
  ggplot(., aes(x=Minus10, y= Minus35)) + geom_tile(aes(fill = log10(RNA_exp_average))) +  
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_gradientn(colors = pal, name = 'Log10(Expression)', limits = c(-1.3,1.3)) +
  labs(title = 'Combinatorial Expression of Core Promoter Elements', x = '-10 Variant', y = '-35 Variant')
ggsave('Figure5B.pdf')
...
inner_join(a,b, by = c('Minus10', 'Minus35', 'Background', 'Spacer')) %>%
  ...
  ggplot(., aes(x=RNA_exp_average.y, y=Fold_UP)) + geom_point(aes(color = Minus35)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
