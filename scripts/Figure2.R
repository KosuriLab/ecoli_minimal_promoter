...
split_exp <- variant_stats %>% 
  separate(col = 'name', into = c("UP_element", "Minus35", "Spacer", "Minus10", 'Background') , sep = "_", remove = F) %>%
  left_join(., min_MOPS, by = 'name') 
#The 'too few values' warning is referring to the negative and positive controls within our library, which do not have these elements and so get removed
...
ggplot(min_MOPS, aes(num_barcodes)) + geom_density(alpha=.2, fill = "#3B9AB2", color = "#3B9AB2") +
...
ggplot(min_MOPS, aes(RNA_exp_rep2, RNA_exp_rep3)) + geom_point(alpha = .4) +
  ...
  xlab('Expression (RNA/DNA) Replicate #1') + ylab('Expression (RNA/DNA) Replicate #2') +
  ggtitle('Comparing Promoter Expression Between Biological Replicates') +
  geom_point(data=test, aes(RNA_exp_rep2, RNA_exp_rep3), color = "firebrick1") + geom_point(data=filter(split_exp, Minus10 == 'consensus10' & Minus35 == 'consensus35'),aes(RNA_exp_rep2,RNA_exp_rep3), color = 'deepskyblue')
ggsave('Figure2E.pdf')
