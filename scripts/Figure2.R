...
min_MOPS <- read.table("../processed_data/revLP5_Min_MOPS_glu_expression.txt", 
                       header = T)

# Take promoter variant names and assign columns indicating which element variants it has
variant_stats <- read.table("../processed_data/variant_statistics.txt", 
                            header = T, sep = "\t")
...
split_exp <- variant_stats %>% 
  separate(col = 'name', 
           into = c("UP_element", "Minus35", "Spacer", "Minus10", 'Background') , 
           sep = "_", remove = F) %>%
...
split_exp <- split_exp[!is.na(split_exp$RNA_exp_average),]

write.table(split_exp, "../processed_data/rlp5Min_SplitVariants.txt", 
            quote = F, row.names = F)
...
ggplot(min_MOPS, aes(num_barcodes)) + 
    ...
    geom_density(aes(num_barcodes_integrated), alpha=.2, fill = "#F21A00", 
                 color = "#F21A00") +
    scale_x_log10() + annotation_logticks(sides = 'b') +
    annotate("text", x=7, y=1.35, 
             label = paste(' Integrated Median =', 
                           median(min_MOPS$num_barcodes_integrated), 
                           'barcodes')) +
    annotate("text", x=7, y=1.43, label = paste('Mapped Median =', 
                                                median(min_MOPS$num_barcodes), 
                                                'barcodes')) +
  labs(x='Number of Mapped Barcodes', 
       y='Density', 
       title = 'Distribution of the Number of Barcodes Per Variant')
ggsave('../figs/Figure2C.pdf')
...
corr <- summary(lm(RNA_exp_rep2 ~ RNA_exp_rep3, min_MOPS))$r.squared
summary(lm(RNA_exp_rep2 ~ RNA_exp_rep3, min_MOPS)) # p < 2.2e-16

ggplot(min_MOPS, aes(RNA_exp_rep2, RNA_exp_rep3)) + 
    geom_point(alpha = .4) +
    ...
    ggtitle('Comparing Promoter Expression Between Biological Replicates') +
    geom_point(data=test, aes(RNA_exp_rep2, RNA_exp_rep3), color = "firebrick1") + 
    geom_point(data=filter(split_exp, Minus10 == 'consensus10' & Minus35 == 'consensus35'),aes(RNA_exp_rep2,RNA_exp_rep3), color = 'deepskyblue')
ggsave('../figs/Figure2E.pdf')
