...
#train model to predict expression based on each individual element and 
#combinations of -10 and -35 elements
linM = train %>%
  select(RNA_exp_log, UP_element, Minus35, Spacer, Minus10, Background) %>%
  lm(formula=RNA_exp_log ~ UP_element + Minus35 + Spacer + Minus10 + Background + Minus10:Minus35, 
     data=., model = T) 
...
ggplot(test, aes(10^predicted_exp_lm, 10^RNA_exp_log, color = Minus35)) +
  ...
  annotate("text", x = .1, y = 10, 
           label = paste('R^2==', signif(corr, 3)), parse = T) +
...
#FIRST TIME
pie_linM <- data.frame(
  Element = c("Minus10", "Minus35", 'UP element', 'Spacer', 'Background', 
              'Minus10:Minus35', 'Residual' ),
...
ggplot(melt, aes(x=subset, y=value)) + stat_boxplot(geom = "errorbar", width = 0.25) + 
  ...
  geom_jitter() + 
  ylim(.75,1) +
  labs(x='Proportion of Data Trained On', 
       y='Correlation between Predicted and Actual Expression', 
       title = '10-Fold Cross Validation with Variable Proportion of Training Data')
ggsave('../figs/Figure4D.pdf')
