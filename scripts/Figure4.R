...
# Log transformed data for lm
# split_exp <- mutate(split_exp, RNA_exp_log = log10(RNA_exp_1)) 
...
pie_linM <- data.frame(
  ...
  value = c(400.00, 546.10,  46.22,  67.49, 15.19, 371.52,346.57) #Mean squares for 
)
...
for (i in c(.05,.1, .2,.3,.4, .5, .6, .7, .8, .9)) {
  ...
  # split_exp$RNA_exp_log <- log10(split_exp$RNA_exp_1)
...
a<- as.numeric(corrz) #DO this AFTER THE FIRST LOOP OR YOU'LL GET ERROR ABOVE^^^
